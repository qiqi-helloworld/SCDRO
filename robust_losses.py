...
# from fastdrosourcecode.fastdrodatasets import CustomDistributionSampler
# from fastdrosourcecode.fastdroutils import project_to_cs_ball, project_to_cvar_ball
...
def chi_square_value(p, v, reg):
    ...
    with torch.no_grad():
        ...
        # xx = torch.ones(m,)
        # print(m, p.size(), torch.ones_like(p, ))
        ...
        if self.geometry == 'cvar':
            if self.reg > 0:
                ...
                def p(eta):
                    x = (v - eta) / reg
                   #print(v, x)
                    min_val = (torch.Tensor([1 / size]).type(x.dtype)) / m
